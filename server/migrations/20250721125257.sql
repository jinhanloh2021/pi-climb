...
AS $$
BEGIN
  UPDATE users SET follower_count = follower_count + 1 WHERE id = NEW.to_user_id;
...
CREATE OR REPLACE FUNCTION handle_soft_delete_follow()
RETURNS trigger
SECURITY DEFINER SET search_path = ''
AS $$
BEGIN
  ...
  IF OLD.deleted_at IS NULL AND NEW.deleted_at IS NOT NULL THEN
    UPDATE users SET follower_count = follower_count - 1 WHERE id = OLD.to_user_id;
...
DROP TRIGGER IF EXISTS trg_soft_delete_follow ON follows;

CREATE TRIGGER trg_soft_delete_follow
BEFORE UPDATE ON follows
FOR EACH ROW
WHEN (OLD.deleted_at IS DISTINCT FROM NEW.deleted_at)
EXECUTE FUNCTION handle_soft_delete_follow();
